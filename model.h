...
/*
* Creating the Model header file to load the model
*/
class Model
{
public:
...
private:
	/*
	* Boolean function and constructor to process Node for Model
	*/
	bool processNode(const aiNode* node, const aiScene* sceneObjPtr)
	{
		...
		// For when number of meshes increases
		...
		// For when number of child node increases
	...
	bool processMesh(const aiMesh* meshPtr, const aiScene* sceneObjPtr, Mesh& meshObj)
	{
		...
		// In creating the number of vertices for the model
		...
		// for when the number of mesh pimitives (triangles, polygons, lines) increases
		...
		if (meshPtr->mMaterialIndex >= 0)
		{
			...
			// Initialise the Specular material for texture
			...
			// Initialise the Normal Mapping material for texture
	...
	/*
	* Boolean Function and Constructor to process material for model
	*/
		...
		const aiTextureType textureType, std::vector<Texture>& textures)
	{
		...
		for (size_t i = 0; i < matPtr->GetTextureCount(textureType); ++i)
		{
			...
			if (it == this->loadedTextureMap.end()) // When Texture Map closes
			{
...
private:
	...
	std::string modelFileDir; // model file directory as a string
	typedef std::map<std::string, Texture> LoadedTextMapType; // key = texture file path
	LoadedTextMapType loadedTextureMap; // texture file path as a variable
};
